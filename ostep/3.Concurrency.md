...
假如有两个线程，其中线程A需要等待线程B完成某个操作之后才能往下执行，一个最简单的实现的实现类似自旋锁，不停检查一个变量的值确定是否完成：
...
这个方法的问题依然是效率，把done替换成一个互斥锁能解决这个问题吗？在这个例子中可以
...
这里的用法稍显怪异，需要在同一个线程中上两次锁，因为互斥锁从设计上就不是用来同步。事实上，存在一些场景无法用互斥锁解决，比如这个生产者/消费者问题（例子来自[stackoverflow](https://stackoverflow.com/questions/12551341/when-is-a-condition-variable-needed-isnt-a-mutex-enough))，线程1需要等待线程2的信号才能往下执行
...
while(1) {
    ... // 准备工作，比如生产一个东西
    unlock(mutex); // 解除线程1的阻塞状态，使其往下执行任务
    lock(mutex); // 上锁阻塞线程1
}
```

这里有两个问题：
1. 线程1在unlock之前，线程2无法继续进行准备工作
2. 更严重的，如果线程1在unlock之后，如果操作系统没有调度运行线程2，它会马上回到循环起点，把自己锁住。好了，死锁了。
