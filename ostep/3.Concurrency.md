...
如果上锁失败，则通过lll_futex_wait()把自己加入到队列中等待唤醒，这个函数的底层是了futex系统调用，内部还有一层测试和等待锁的逻辑，因此不会出现测试和等待之间线程被调度出去的情况。关于futex的更多详情可以参考[linux内核级同步机制--futex](https://github.com/farmerjohngit/myblog/issues/8).

> 如果这种情况出现在用户态，如这里的trylock()和wait()线程被调度出去了，并且另一个线程在此时释放了锁，这个线程将永远等待下去。
> ```
>     while (!trylock(lock)) {
>         wait(lock);  
>     }
