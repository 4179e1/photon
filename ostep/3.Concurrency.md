...
pthread是怎么实现锁这个机制的呢？ 下面假设我们要实现一个类似phread的线程库，因此请注意以下的伪代码都是运行在用户态而非内核态的。
...
如果上锁失败，则通过lll_futex_wait()把自己加入到队列中等待唤醒，这个函数内部还会有一层测试和等待锁的逻辑，但是这个函数的底层是在内核中实现的，因此不会出现测试和等待之间间线程被调度出去的情况。如果在用户态出现这种情况，并且另一个线程在此时释放了锁，这个线程将永远等待下去。关于futex的更多详情可以参考[linux内核级同步机制--futex](https://github.com/farmerjohngit/myblog/issues/8)
