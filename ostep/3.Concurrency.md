...
如果上锁失败，则通过lll_futex_wait()把自己加入到队列中等待唤醒，这个函数的底层是在内核态实现的，因此不会出现测试和设置一个值之间线程被调度出去的情况。关于futex的更多详情可以参考[linux内核级同步机制--futex](https://github.com/farmerjohngit/myblog/issues/8)
